...
|---|---|
| `registerApexTestWireAdapter` / `registerLdsTestWireAdapter` | `vi.mock('@salesforce/platform-sdk', () => ...)` with a `vi.fn()` graphql |
...
| `Cannot find module '@salesforce/platform-sdk'` in tests | Vitest needs a `vi.mock(...)` hoist; verify it's at the top of the file |
